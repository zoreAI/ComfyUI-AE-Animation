...
class AeTimelineView {
    ...
    load(animation) {
        ...
        Object.assign(this.ui.canvas, {
            width: project.width || 1920,
            height: project.height || 1080
        });
        this.ui.info.textContent = `${this.ui.canvas.width}×${this.ui.canvas.height} | ${this.state.fps}fps | ${this.state.duration.toFixed(1)}s`;
        this.ui.frameCount.value = this.state.totalFrames;

        this.state.layers = (animation.layers || []).map((layer) => ({
            ...layer,
            x: layer.x || 0,
            y: layer.y || 0,
            scale: layer.scale || 1,
        ...
        this.#loadImages().then(() => {
            ...
            if (select.options.length) {
                this.state.selectedId = select.value = (this.state.selectedId && this.state.layers.find((l) => l.id === this.state.selectedId))
    ...
    #setupWidgets() {
        ...
        const widgets = {
            keyframes: keyWidget,
            totalFrames: this.node.widgets?.find((w) => w.name === "total_frames"),
    ...
    #buildUI() {
        ...
        frameLabel.textContent = "总帧数:";
        frameLabel.style.marginLeft = "auto";
        const frameInput = document.createElement("input");
        Object.assign(frameInput, { type: "number", min: 1, value: 150, step: 1 });
        ...
        infoRow.append(infoText, frameLabel, frameInput);
        root.append(infoRow);
        ...
        layerRow.append(layerSelect, layerUpBtn, layerDownBtn, layerTopBtn, layerBottomBtn, bgSelect);
        ...
        ctrlRow.append(timeInput, goBtn, addBtn, delBtn, clrBtn, maskBtn, maskBrushLabel, maskBrushInput, maskBrushValue, maskApplyBtn, pathBtn, pathAddBtn, pathApplyBtn, extractBtn, extractBrushLabel, extractBrushInput, extractBrushValue, extractBlurLabel, extractBlurSelect, extractApplyBtn, playBtn, stopBtn, refreshBtn, timeLabel);
        root.append(ctrlRow);
    ...
    #bindEvents() {
        ...
        ui.frameCount.addEventListener("input", () => {
            const total = parseInt(ui.frameCount.value, 10);
            if (!Number.isNaN(total) && total > 0) {
                this.state.totalFrames = total;
                this.state.duration = total / this.state.fps;
                // Sync to widget
                if (this.widgets.totalFrames) {
                    this.widgets.totalFrames.value = total;
                }
                this.#drawTimeline();
                this.#persist();
            }
    ...
    #persist(includeImageCache = false) {
        ...
        this._persistTimer = setTimeout(() => {
            const payload = this.state.layers.map((layer) => {
                ...
                if ((includeImageCache || isExtracted) && layer.image_data) {
    ...
    #refreshFromCache() {
        // Reload from widget cache without executing node
        this.#loadFromWidget();
        alert("已从缓存刷新预览");
    }
        ...
        api.addEventListener("ae_animation_update", (event) => {
            ...
            if (view && detail.animation) {
                try {
                    ...
                    // Only load if widget is empty (first execution) or user explicitly refreshed
                    // This prevents overwriting user's UI modifications with backend data
                    const hasWidgetData = view.widgets.keyframes?.value && view.widgets.keyframes.value !== "[]";
                    if (!hasWidgetData) {
                        console.log("[AE] Loading from WebSocket (widget empty)");
                        view.load(payload);
                    } else {
        ...
        nodeType.prototype.onNodeCreated = function onNodeCreated() {
            ...
            this.onExecuted = (output) => {
                ...
                try {
                    ...
                    // Only load if widget is empty (first execution)
                    // User modifications are already persisted to widget, so don't reload
                    const hasWidgetData = view.widgets.keyframes?.value && view.widgets.keyframes.value !== "[]";
                    if (!hasWidgetData) {
                        console.log("[AE] Loading from onExecuted (widget empty)");
                        view.load(json);
                    } else {
