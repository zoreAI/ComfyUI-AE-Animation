...
### 🎨 高级功能
- **Mask画笔**：自定义图层透明度遮罩，支持抖动边缘效果
- **贝塞尔路径**：路径动画和运动轨迹
- **背景提取（Extract）**：画笔涂抹提取背景区域，自动模糊背景并创建新前景层
- **图层排序**：上移/下移/置顶/置底
- **本地图片导入**：UI内直接导入本地图片作为新图层，自动压缩优化
- **单节点预览**：无需运行整个工作流，可单独执行预览动画
- **UI参数控制**：分辨率、FPS、总帧数可在UI内统一调整
...
**输入**：
- `width`, `height`：项目尺寸（默认1280x720，建议在UI内调整）
- `fps`：帧率（默认16，建议在UI内调整）
- `total_frames`：总帧数（默认81，建议在UI内调整）
- `mask_expansion`：Mask扩展/收缩（-10到10）
- `mask_feather`：Mask羽化（0到20）
- `background_image`：背景图片（可选）
- `foreground_images`：前景图片（可选，支持批量输入）
- `ui_preview_only`：设为True时仅输出预览数据
...
1. **分辨率控制**：直接在UI输入宽×高（预览最大1920×1080，导出使用实际分辨率）
2. **FPS设置**：调整动画帧率（1-120）
3. **总帧数**：设置动画总长度
4. **应用按钮**：统一应用分辨率、FPS、总帧数设置
5. **导入图片**：`＋图片` 按钮导入本地图片作为新图层
6. **运行预览**：`▶ 运行` 按钮单独执行此节点预览动画
7. **清理缓存**：`🗑 清理缓存` 释放未选中图层的图片内存
...
### 性能优化
- **智能压缩**：导入图片自动压缩至画布分辨率，减少内存占用
- **懒加载**：图片按需加载，避免浏览器崩溃
- **缓存管理**：手动清理未使用图层缓存，释放内存
- **预览限制**：预览画布最大1920×1080，大分辨率项目保持流畅
- **Debounce机制**：减少不必要的保存操作（300ms）
...
1. **项目尺寸**：建议在UI内调整分辨率，预览限制1920×1080，导出使用实际分辨率
2. **图层数量**：前景图层数量 = foreground_images的批次数 + Extract创建的图层 + 本地导入图层
3. **Mask分辨率**：Mask会自动缩放到图层原始尺寸
4. **Extract图层**：创建后会自动缩放到项目尺寸，scale=1即为全屏显示
5. **内存管理**：大量图层时使用清理缓存功能释放内存
6. **外部图层**：通过节点输入口连接的foreground_images会自动显示在UI中

## 更新日志

### v1.1.0（最新）
- ✅ **UI内导入图片**：支持本地图片导入，自动压缩优化
- ✅ **Mask抖动边缘**：可调强度的不规则边缘效果
- ✅ **UI参数控制**：分辨率、FPS、总帧数统一在UI内调整
- ✅ **单节点预览**：无需运行整个工作流即可预览动画
- ✅ **性能优化**：图片压缩、懒加载、缓存管理
- ✅ **预览限制**：预览画布最大1920×1080，防止浏览器崩溃
- ✅ **外部图层显示**：自动识别并显示节点输入口的前景图片
- ✅ **默认参数调整**：1280×720、16fps、81帧
- ✅ 移除多边形遮罩功能
